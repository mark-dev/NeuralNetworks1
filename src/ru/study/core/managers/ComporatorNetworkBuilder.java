...
import ru.study.core.neurons.VirtualNeuron;
import ru.study.core.neurons.WhiteNeuron;
...
public class ComporatorNetworkBuilder {
    ...
    public ComporatorInfo addNewComporator(Neuron a, Neuron b) {
        comporatorCount++;
        ...
        Neuron a1 = new BlackNeuron(Neuron.NEURON_TYPE_FINAL, 1, comporatorId("a1"));
        Neuron b1 = new BlackNeuron(Neuron.NEURON_TYPE_FINAL, 1, comporatorId("b1"));
    ...
    //Причем мы ее строим не с нуля, а имя уже какой-то компоратор высшего уровня
    private ComporatorInfo non2powComparatorNetwork(double[] values, ComporatorInfo topLevel) {
        if (values.length == 1) {
            Neuron lastNeuron = new VirtualNeuron(values[0], "direct!");
        ...
        System.out.println("pow2Values: " + Arrays.toString(pow2values));
        ...
        for (int i = 0; i < levelCount; i++) {
            ...
            if (i == 0) {
                for (int q = 0; q < values.length - 1; q = q + 2) {
                    Neuron a = new VirtualNeuron(values[q], "value" + q);
                    Neuron b = new VirtualNeuron(values[q + 1], "value" + (q + 1));
